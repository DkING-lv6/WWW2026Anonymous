...
def load_mat(dataset, train_rate=0.3, val_rate=0.1):
    ...
    return adj, feat, ano_labels, all_idx, idx_train, idx_val, idx_test, ano_labels, str_ano_labels, attr_ano_labels, normal_label_idx


def adj_to_dgl_graph1(adj):
    ...
    src = torch.tensor(adj.row, dtype=torch.long).to(device)
    dst = torch.tensor(adj.col, dtype=torch.long).to(device)
